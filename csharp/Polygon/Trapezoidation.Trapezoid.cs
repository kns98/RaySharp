...
public partial class Trapezoidation
{
    ...
    private class Trapezoid
    {
        ...
        ///     Gets a debug string
        /// </summary>
        public string Debug => $"Left:{leftVertex} {leftBase} Low: {lowerEdge} High: {upperEdge}";
        ...
        /// <param name="upperEdge">the upper edge of the new split</param>
        /// <param name="splitSink">the polygon splitter</param>
        public void LeaveInsideBySplit(int vertexId, TrapezoidEdge lowerEdge, TrapezoidEdge upperEdge,
            IPolygonSplitSink splitSink)
        {
            EvaluateRight(vertexId, Base.TwoNeighbors, splitSink);
        ...
        {
            EvaluateRight(vertexId, Base.NoNeighbor, splitSink);
        }
        ...
        {
            EvaluateRight(vertexId, Base.UpperCorner, splitSink);
            UpdateEdges(new Trapezoid(vertexId, Base.UpperCorner, lowerEdge, nextEdge));
        }
        ...
        {
            EvaluateRight(vertexId, Base.LowerCorner, splitSink);
            UpdateEdges(new Trapezoid(vertexId, Base.LowerCorner, nextEdge, upperEdge));
        }
        ...
        {
            var combinedBase = leftBase | rightBase;
            if (DetectDoubleNeighbor(combinedBase) || DetectDiagonale(combinedBase))
                splitter.SplitPolygon(leftVertex, rightVertex);
        }
