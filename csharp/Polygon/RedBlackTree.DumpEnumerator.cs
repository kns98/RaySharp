...
public sealed partial class RedBlackTree<T>
{
    ...
    private class DumpEnumerator : IEnumerable<string>
    {
        ...
        {
            this.tree = tree;
            configuredDept = maxDepth;
        }

        /// <inheritdoc />
        public IEnumerator<string> GetEnumerator()
        {
            foreach (var line in Dump()) yield return line;
        }
        ...
        private static IEnumerable<(T data, string color, int level)> EnumerateLevels(Node node, int level,
            int maxDepth)
        {
            Node left;
            Node right;
            if (node == null)
            {
                if (maxDepth < 0 || level > maxDepth) yield break;
                left = null;
                right = null;
                yield return (default, "_", level);
            }
            else
            {
                left = node.Left;
            ...
            foreach (var item in EnumerateLevels(left, level + 1, maxDepth)) yield return item;
            foreach (var item in EnumerateLevels(right, level + 1, maxDepth)) yield return item;
        }
        ...
        {
            var maxDepth = configuredDept;
            if (maxDepth < 0)
            {
                if (tree.Count == 0) return Enumerable.Empty<string>();
                maxDepth = EnumerateLevels(tree.root, 0, -1).Select(x => x.level).Max();
            }

            var groups = EnumerateLevels(tree.root, 0, maxDepth)
                .GroupBy(x => x.level)
                .OrderBy(x => x.Key)
                .ToArray();
            ...
            return groups.Select(g =>
            {
                ...
                var rest = string.Join(spacing,
                    g.Select(x =>
                        (Equals(x.data, default(T)) && !char.IsLower(x.color[0]) ? "- " : $"{x.data}{x.color}").PadLeft(
                            itemLenght)));
                return left + rest;
            });
        }
