...
public partial class Trapezoidation
{
    ...
    private class EdgeComparer : IComparer<TrapezoidEdge>
    {
        ...
        public int Compare(TrapezoidEdge x, TrapezoidEdge y)
        {
            ...
            return IsVertexAbove(vertexOfValue, storage) ? 1 : -1;
        }
        ...
        {
            var left = vertices[upper.Left];
            var upperRight = vertices[upper.Right];
            var lowerRight = vertices[lower.Right];
            ...
            var lowerRightY = lowerRight.Y;
#endif
            if (upperRightY > leftY != lowerRightY > leftY) return upperRightY > lowerRightY;
...
#else
            var xSpan = right.X - left.X;
            if (xSpan < Epsilon * Epsilon) return vertex.Y > left.Y;
            var yOfEdgeAtVertex = (vertex.X - left.X) / xSpan * (right.Y - left.Y) + left.Y;
            return yOfEdgeAtVertex < vertex.Y;
#endif
        }
        ...
        private bool IsVertexAbove(int vertexId, TrapezoidEdge edge)
        {
            ...
            if (vertex.X >= left.X)
            {
                if (vertex.Y > left.Y)
                {
                    if (left.Y >= right.Y || (vertex.X < right.X && vertex.Y > right.Y)) return true;
                }
                else
                {
                    if (left.Y < right.Y || (vertex.X < right.X && vertex.Y < right.Y)) return false;
                }
            }
#endif
