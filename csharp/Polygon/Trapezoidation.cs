...
public partial class Trapezoidation
{
    ...
    /// <summary>
    ///     map the right of the vertex to the active polygon edge. Collisions for closing vertices ar handled by
    ///     <see cref="EdgeForVertex" />
    /// </summary>
    ...
    {
        vertexToEdge = new Dictionary<int, TrapezoidEdge>();
        comparer = new EdgeComparer(vertices);
        activeEdges = new RedBlackTree<TrapezoidEdge>(comparer);
    ...
    /// </summary>
    internal IEnumerable<ITestingTrapezoidEdge> Edges => activeEdges.Items;
    ...
    {
        var (lowerEdge, upperEdge) = StartNewTrapezoidEdges(
            info.Id,
            info.PrevVertexId,
            info.PrevUnique,
            info.NextVertexId,
        ...
        else
        {
            ...
            trapezoid.LeaveInsideBySplit(info.Id, lowerEdge, upperEdge, splitSink);
        }
    }
    ...
    {
        var lowerEdge = EdgeForVertex(info.Id, info.Unique);
        TrapezoidEdge upperEdge;
        var prevEdge = lowerEdge.TreeNode.PrevNode?.Data;
        if (prevEdge?.Right == lowerEdge.Right &&
            (prevEdge.Left == info.PrevVertexId || prevEdge.Left == info.NextVertexId))
        {
            upperEdge = lowerEdge;
            lowerEdge = prevEdge;
        }
        ...
        if (lowerEdge.IsRightToLeft)
        {
            lowerTrapezoid.LeaveInsideByJoin(info.Id, splitSink);
        }
        else
        {
            var upperEdge2 = lowerEdge.TreeNode.NextNode.Data;
            var upperTrapezoid = upperEdge2.Trapezoid;
            Trapezoid.EnterInsideByJoin(lowerTrapezoid, upperTrapezoid, info.Id, splitSink);
        }

        JoinTrapezoidEdges(lowerEdge);
    }
    ...
    {
        var oldEdge = EdgeForVertex(info.Id, info.Unique);
        var trapezoid = oldEdge.Trapezoid;
        if (oldEdge.IsRightToLeft)
        {
            var newEdge = Transition(oldEdge, info.PrevVertexId, info.PrevUnique);
            trapezoid.TransitionOnLowerEdge(info.Id, newEdge, splitSink);
        }
        else
        {
            var newEdge = Transition(oldEdge, info.NextVertexId, info.NextUnique);
            trapezoid.TransitionOnUpperEdge(info.Id, newEdge, splitSink);
        }
    }
    ...
    {
        return StartNewTrapezoidEdges(start, prev, prev, next, next);
    }
    ...
    {
        if (edge is TrapezoidEdge trapezoidEdge) return Transition(trapezoidEdge, newTarget, newTarget);
    ...
    internal void TestJoin(ITestingTrapezoidEdge lower)
    {
        ...
        {
            JoinTrapezoidEdges(trapezoidEdge);
            return;
        }
    ...
    ///     There is never a Begin() where the prev or next is left to start or prev is at same X and below start.
    /// </remarks>
    private (TrapezoidEdge lower, TrapezoidEdge upper) StartNewTrapezoidEdges(int start, int prev, int prevUnique,
        int next, int nextUnique)
    {
        ...
        if (!comparer.EdgeOrderingWithCommonLeftIsCorrect(lower, upper)) (lower, upper) = (upper, lower);
        (lower.TreeNode, upper.TreeNode) = activeEdges.AddPair(lower, upper);
        StoreEdge(lower);
        StoreEdge(upper);
        return (lower, upper);
    }
    ...
    private TrapezoidEdge Transition(TrapezoidEdge edge, int newTarget, int targetUnique)
    {
        ...
        nextEdge.TreeNode = activeEdges.ReplaceNode(edge.TreeNode, nextEdge);
        vertexToEdge.Remove(edge.RightUnique);
        StoreEdge(nextEdge);
        return nextEdge;
    }
    ...
    private void JoinTrapezoidEdges(TrapezoidEdge lowerEdge)
    {
        ...
        activeEdges.RemoveNode(nextNode);
        activeEdges.RemoveNode(lowerEdge.TreeNode);
        vertexToEdge.Remove(lowerEdge.RightUnique);
    }
    ...
    {
        vertexToEdge[edge.RightUnique] = edge;
    }
    ...
    {
        if (!vertexToEdge.TryGetValue(vertexUniqe, out var edge))
