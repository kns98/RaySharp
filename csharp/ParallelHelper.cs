...
public static class ParallelHelper
{
    ...
    ///     The type of action (implementing <see cref="IAction2D" />) to invoke for each pair of
    ///     iteration indices.
    /// </typeparam>
    ...
    public static void For2D<TAction>(int top, int bottom, int left, int right, in TAction action,
        ...
        where TAction : IAction2D
    {
        if (minimumActionsPerThread <= 0) throw new Exception();
        if (top > bottom) throw new Exception();
        ...
        var batchHeight = 1 + (height - 1) / numBatches;
...
/// </summary>
/// <remarks>
///     If the <see cref="Invoke" /> method is small enough, it is highly recommended to mark it with
