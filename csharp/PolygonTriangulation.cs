...
public class PolygonTriangulator
{
    ...
    public static List<List<PointF>> Triangulate(List<PointF> Polygon, bool triangulate = false)
    {
        ...
        while (N >= 3)
        {
            while (PMSquare(tempPolygon[begin_ind], tempPolygon[(begin_ind + 1) % N],
                   ...
                   Intersect(tempPolygon, begin_ind, (begin_ind + 1) % N, (begin_ind + 2) % N))
            {
                begin_ind++;
                begin_ind %= N;
            }
            ...
            if (triangulate == false)
            {
                ...
                {
                    if (Intersect(tempPolygon, begin_ind, (cur_ind + 1) % N, (cur_ind + 2) % N) ||
    ...
    public static int SelfIntersection(List<PointF> polygon)
    {
        if (polygon.Count < 3)
            return 0;
        var High = polygon.Count - 1;
        var O = new PointF();
        int i;
        for (i = 0; i < High; i++)
        for (var j = i + 2; j < High; j++)
    ...
    public static float Square(List<PointF> polygon)
    {
        ...
        if (polygon.Count >= 3)
        {
            ...
            S += PMSquare(polygon[polygon.Count - 1], polygon[0]);
        }
    ...
    private static float PMSquare(PointF p1, PointF p2, PointF p3)
    {
    ...
    private static int LineIntersect(PointF A1, PointF A2, PointF B1, PointF B2, ref PointF O)
    {
        var a1 = A2.Y - A1.Y;
        var b1 = A1.X - A2.X;
        var d1 = -a1 * A1.X - b1 * A1.Y;
        var a2 = B2.Y - B1.Y;
        var b2 = B1.X - B2.X;
        var d2 = -a2 * B1.X - b2 * B1.Y;
        var t = a2 * b1 - a1 * b2;
        if (t == 0)
            return -1;
        O.Y = (a1 * d2 - a2 * d1) / t;
